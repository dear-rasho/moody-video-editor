...
//  Applies ALL active effect layers to the preview canvas:
//    1. CSS filter composition (from 'filter' + 'effect' kinds)
//    2. CSS transform composition (motion: shake, bounce, pulse…)
//    3. Pixel effects (adjustments / colorWheel / chroma)
//  Skips effect layers on HIDDEN tracks.
// ================================================================

const CSS_ID = 'effect-renderer-styles';
let rafPending = false;
let lastCssFilter = '';
let lastCssTransform = '';
...
// ─── Find active effect layers at time ────────────────────────
...
function apply() {
  ...
  // ─── 2) CSS TRANSFORMS (motion effects) ───────────────────
  let transform = '';
  for (let i = 0; i < active.length; i++) {
    const st = active[i].clip.effectState;
    if (!st || !st.motion) continue;
    const m = computeMotion(st.motion, time);
    if (m) transform = transform ? transform + ' ' + m : m;
  }
  if (transform !== lastCssTransform) {
    if (transform) canvas.style.setProperty('transform', transform, 'important');
    else canvas.style.removeProperty('transform');
    lastCssTransform = transform;
  }
