...
//  Speed control panel. Delegates all live playback to
//  keyframeEngine.js — that's why this file is now smaller.
//
//  Every change (slider, preset, keyframe, easing) calls
//  registerSpeed(clip) so the engine picks it up globally.
// ================================================================
...
function injectStyles() {
  ...
  s.textContent = [
    ...
    '.sp-ease-card{flex:0 0 96px;width:96px;height:78px;padding:5px 5px 4px;background:var(--surface);border:1px solid var(--border);border-radius:9px;color:var(--text);cursor:pointer;display:flex;flex-direction:column;gap:4px;scroll-snap-align:start;font-family:inherit;transition:border-color .12s ease,box-shadow .12s ease;}',
    '.sp-ease-card:active{background:var(--surface-2);}',
    ...
    '.sp-reset:active{background:var(--surface-2);}',
    ...
    '.sp-empty.warn{border-color:#7f1d1d;background:#231010;}',
    '.sp-empty.warn .sp-empty-title{color:#ff9e9e;}'
  ].join('\n');
...
function syncEngine(clip) {
  // Delegate to the global engine — it applies during playback
  registerSpeed(clip);
}
...
export function renderTo(container) {
  ...
  // Register with the global engine immediately
  syncEngine(sel.clip);
  ...
  slider.addEventListener('input', () => {
    onSpeedChange(parseFloat(slider.value));
  });
...
// ─── Keyframe section (easing shelf above list) ───────────────
function buildKeyframeSection(sel) {
  ...
  addBtn.addEventListener('click', () => {
    ...
    const filtered = st.kf.filter(k => Math.abs(k.time - t) > 0.05);
  ...
  previewBtn.addEventListener('click', () => previewSpeed(sel));

  actions.append(previewBtn, addBtn);
  section.appendChild(actions);

  // ⬆ Easing shelf FIRST (mobile-friendly, visible without scrolling)
...
function buildEasingSelector(sel) {
  ...
  EASING_OPTIONS.forEach(opt => {
    ...
    dotA.setAttribute('cx', '4');   dotA.setAttribute('cy', '44');
...
// ─── Build SVG curve path (uses shared easing math) ───────────
...
// ─── Preview (engine handles live rate) ───────────────────────
function previewSpeed(sel) {
  const video = document.querySelector('#preview-video');
  if (!video) return;

  // Ensure this clip is loaded in the preview
  if (!video.src || !sel.clip.url || video.src !== sel.clip.url) {
    showToast('Open this clip in the preview first', false);
    return;
  }
...
function buildEmptyState(opts) {
  ...
  wrap.className = 'sp-empty' + (opts.warn ? ' warn' : '');
